...
def update_all_player_info(
    ...
    if (all_player_info.get((player, time_control)) is None) & (current_rating != 1500.0):
    ...
    elif (all_player_info.get((player, time_control)) is None) & (current_rating == 1500.0):
        pass
    ...
    # so update or append each field as needed 
        ...
        all_player_info[(player, time_control)]['opponent_ratings'].append(opponent_rating)
...
while(True):
    ...
    white_gain, black_gain = headers.get('WhiteRatingDiff'), headers.get('BlackRatingDiff')
    ...
    skip_game_condition = (
        ('?' in white_player) | ('?' in black_player) | 
        (white_player is None) | (black_player is None) | 
        ('?' in str(white_rating)) | ('?' in str(black_rating)) | 
        (white_gain is None) | (black_gain is None) | 
